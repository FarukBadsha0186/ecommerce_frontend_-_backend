...
function Dashboard() {
  ...
  // ✅ fetchDashboardStats আগে define করো
  const fetchDashboardStats = useCallback(async () => {
    try {
      ...
      if (response.data.success && response.data.data) {
        ...
        setStats(prevStats =>
          prevStats.map(stat => {
            let value = 0;
            switch (stat.key) {
              case "totalUsers": value = summary.totalUsers || 0; break;
              case "totalProducts": value = summary.totalProducts || 0; break;
              case "totalOrders": value = totalOrders; break;
              case "totalIncome": value = `$${(summary.totalIncome || 0).toFixed(2)}`; break;
              case "pendingOrders": value = orderStatus.pending || 0; break;
              case "deliveredOrders": value = orderStatus.delivered || 0; break;
              case "cancelledOrders": value = orderStatus.cancelled || 0; break;
              case "totalCategories": value = summary.totalCategories || 0; break;
              case "totalBrands": value = summary.totalBrands || 0; break;
              case "totalReviews": value = data.totalReviews || 0; break;
              default: value = 0;
            }
            return { ...stat, value };
          })
      ...
      if (err.response?.status === 401) {
        ...
        setError("Dashboard API not found.");
      } else {
        setError(err.response?.data?.message || "Failed to fetch dashboard data");
      }
    } finally {
    ...
    return (
      <div>
        ...
        <div className="bg-red-900/50 border border-red-500 rounded-xl p-6 text-center">
          <p className="text-red-400 mb-4">⚠️ {error}</p>
          <div className="flex gap-3 justify-center">
            <button onClick={fetchDashboardStats} className="px-4 py-2 bg-blue-600 hover:bg-blue-700 rounded-lg text-white transition">
              🔄 Try Again
            </button>
            <button onClick={() => { localStorage.clear(); navigate("/login"); }} className="px-4 py-2 bg-gray-600 hover:bg-gray-700 rounded-lg text-white transition">
              🚪 Go to Login
            </button>
          </div>
        </div>
